...
def load_house_graph():
    G = nx.Graph()
    # 2 bedrooms house connections for couple
...
# === User behavior profile for couple in 2-bedroom house ===
user_profile = {
    ...
    "resident2_solo_departure": (time(8, 0), time(9, 0)), # 08:00-09:00: Only Resident 2
    "resident1_solo_return": (time(17, 0), time(18, 0)),  # 17:00-18:00: Only Resident 1
    ...
    # Primary locations - both use Bedroom1 as shared bedroom
    "shared_bedroom": "Bedroom1",  # Both sleep here
    "common_areas": ["LivingRoom", "Kitchen"],  # Main activity areas
    "occasional_areas": ["Bedroom2", "ServiceArea"],  # Less frequent use
...
def get_activity_period(ts):
    ...
    elif user_profile["resident2_solo_departure"][0] <= current_time < user_profile["resident2_solo_departure"][1]:
        return "resident2_solo_departure"
    elif user_profile["resident1_solo_return"][0] <= current_time < user_profile["resident1_solo_return"][1]:
        return "resident1_solo_return"
...
def validate_activity_pattern(location, activity_period):
    ...
    elif activity_period in ["resident1_solo_morning", "resident2_solo_departure", "resident1_solo_return", "resident2_solo_night"]:
    ...
    # During sleep time, minimal activity expected (some night movement allowed for couples)
        ...
        return location == user_profile["shared_bedroom"]  # Only bedroom activity during sleep
...
def validate_row(row, graph, sensor_locations, previous_room, motion_tracker, activity_tracker):
    ...
    if is_both_sleep_time(ts, user_profile["both_sleep_hours"]):
        # For couple, some night motion might be allowed but should be minimal and in bedroom
        if has_motion(row) and current_location != user_profile["shared_bedroom"]:
            errors.append(f"Motion outside bedroom during sleep hours: {current_location} at {timestamp_str} (expected only in {user_profile['shared_bedroom']})")
            ...
            if current_location != user_profile["shared_bedroom"]:
                errors.append(f"Non-environmental event outside bedroom during sleep: {current_location} at {timestamp_str}")
    ...
    if not validate_activity_pattern(current_location, activity_period):
        if activity_period == "house_empty":
            ...
            errors.append(f"Activity outside bedroom during sleep: {current_location} at {timestamp_str}")
...
def validate_motion_power_response(motion_events, power_events):
    ...
    # Allow some tolerance - for couples, some power correlation is expected but not all
    if len(motion_events) > 0:
...
def create_motion_frequency_graph(motion_tracker, output_file="motion_frequency_analysis_2_bedrooms_house_couple.png"):
    ...
    # Add activity period markings with correct schedule
    ...
    plt.axvspan(8, 9, alpha=0.1, color='purple', label='R2 Solo Departure')
    plt.axvspan(17, 18, alpha=0.1, color='cyan', label='R1 Solo Return')
    plt.axvspan(18, 22.5, alpha=0.1, color='blue', label='Both Active Evening')
    plt.axvspan(22.5, 23, alpha=0.1, color='magenta', label='R2 Solo Night')
    
    plt.xlabel('Hour of Day')
    plt.ylabel('Number of Motion Events')
    plt.title('Motion Frequency Throughout the Day - 2 Bedrooms House (Couple)')
    plt.xticks(hours)
...
if __name__ == "__main__":
    csv_path = "iot_dataset_2_bedrooms_house_couple_errors.csv"  # Change this path to your CSV file
    error_count, error_log, motion_stats, graph_file = validate_csv(csv_path)

    print(f"\n=== IoT Dataset Validation Results ===")
    print(f"Profile: Couple in 2 Bedrooms House")
    ...
    print(f"Both sleep in Bedroom1")
    print(f"")
    print(f"Activity Timeline:")
    print(f"  06:00-07:00 → Only Resident 1 active")
    print(f"  07:00-08:00 → Both residents active")
    print(f"  08:00-09:00 → Only Resident 2 active")
    print(f"  09:00-17:00 → House empty")
    print(f"  17:00-18:00 → Only Resident 1 active")
    print(f"  18:00-22:30 → Both residents active")
    print(f"  22:30-23:00 → Only Resident 2 active")
