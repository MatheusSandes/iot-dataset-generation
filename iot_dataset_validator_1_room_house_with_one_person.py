...
def load_house_graph():
    G = nx.Graph()
    # 1 room house connections for one person
...
def validate_row(row, graph, sensor_locations, previous_room, motion_tracker, activity_tracker):
    ...
    # Rule 4: Movement between rooms must follow graph structure
    ...
    # Rule 5: Duplicate motion event in the same location at the same second
...
def analyze_motion_frequency(motion_tracker):
    ...
    # Rule 6: Night motion frequency analysis
    ...
    # Rule 7: Work hours motion analysis
...
def create_motion_frequency_graph(motion_tracker, output_file="motion_frequency_analysis_1_room_house_with_one_person.png"):
    ...
    plt.title('Motion Frequency Throughout the Day - 1 Room House (One Person)')
    plt.xticks(hours)
...
def validate_csv(filepath, generate_graph=True):
    ...
    for _, row in df.iterrows():
        ...
        # Rule 8: Check for duplicate event IDs
        event_id = row["event_id"]
    ...
    # Add frequency analysis errors to the log
    for freq_error in frequency_errors:
        error_count += 1
        error_log.append({
            "timestamp": "FREQUENCY_ANALYSIS",
            "location": "OVERALL",
            "event_id": "FREQ_" + str(len(error_log)),
            "errors": [freq_error]
        })
...
if __name__ == "__main__":
    csv_path = "iot_dataset_validator_1_room_house_with_one_person.csv"  # Change this path to your CSV file
    error_count, error_log, motion_stats, graph_file = validate_csv(csv_path)

    print(f"\n=== IoT Dataset Validation Results ===")
    print(f"Profile: One Person in 1 Room House")
    print(f"Sleep hours: 22:30-06:00 (no events allowed)")
    print(f"Work hours: 08:00-18:00 (no events allowed - house empty)")
    print(f"Active hours: 06:00-08:00 and 18:00-22:30")
